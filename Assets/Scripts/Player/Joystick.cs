...
public class Joystick : Controller, IDragHandler, IEndDragHandler //IDragHandler es una interfaz que sirve para detectar el agarre - IEndHandler sirve para decirme cuando termino el agarre (Vienen con Unity)
{
    ...
    public void OnDrag(PointerEventData eventData) //eventData es el que me dice que es lo que esta pasando en la pantalla, en este caso sería el dedo (Por ser un juego Mobile)
    {
        ...
        moveDir = Vector3.ClampMagnitude((Vector3)eventData.position - initialPosition, _maxMagnitude);


        transform.position = initialPosition + moveDir; //Que se mueva a partir del inicio (initialPosition) + la nueva direccion (_moveDir) 
    }

    public float MoveDirX()
    {
        return moveDir.normalized.x;
    }
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        moveDir = Vector3.zero; //Lo igualo a cero para q se detenga
    }
    ...
    public override Vector3 GetMovementInput()
    {
        ...
        Vector2 moreDirModified = new Vector3(moveDir.x, 0f, 0f);
